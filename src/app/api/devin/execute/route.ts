...
export async function POST(request: NextRequest) {
  console.log(`[API /api/devin/execute] POST received`);
  ...
  try {
    ...
    console.log(`[API /api/devin/execute] issue=#${issue?.number} repo=${repo} actionPlan=${Array.isArray(actionPlan) ? actionPlan.length + " steps" : "invalid"}`);

    if (!issue?.number || !repo || !REPO_REGEX.test(repo)) {
      console.warn(`[API /api/devin/execute] Invalid request: issue.number=${issue?.number}, repo=${repo}`);
    ...
    if (!Array.isArray(actionPlan) || actionPlan.length === 0) {
      console.warn(`[API /api/devin/execute] Invalid actionPlan`);
      return NextResponse.json(
    ...
    console.log(`[API /api/devin/execute] Prompt built, length=${prompt.length}`);
    ...
    console.log(`[API /api/devin/execute] Session created: ${JSON.stringify(result)}`);
    return NextResponse.json(result);
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Internal server error";
    console.error(`[API /api/devin/execute] Error: ${message}`);
